    ...
    return (
      <div className="flex flex-col gap-1.5">
        ...
        <div className="relative flex items-center">
          {prefix && (
            <span className="absolute left-3.5 text-sm font-medium text-ink/60 pointer-events-none">
              {prefix}
            </span>
          )}
          <input
            ref={ref}
            className={clsx(
              'w-full rounded-xl border bg-surface-3 px-4 py-3 text-sm text-ink placeholder-ink/20 outline-none transition',
              'focus:border-brand-500/60 focus:ring-1 focus:ring-brand-500/30',
              error ? 'border-accent-rose/50' : 'border-ink/10',
              prefix && 'pl-9',
              suffix && 'pr-9',
              className
            )}
          ...
          />
          {suffix && (
            <span className="absolute right-3.5 text-sm font-medium text-ink/60 pointer-events-none">
              {suffix}
            </span>
          )}
        </div>
