...
export function UserMenu({ syncStatus }: UserMenuProps) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className="relative flex h-9 w-9 items-center justify-center rounded-xl overflow-hidden border border-ink/10 transition hover:border-ink/20"
      >
        {user?.photoURL ? (
      ...
      {/* Dropdown */}
      {open && (
        <div className="absolute right-0 top-11 z-50 w-56 rounded-2xl border border-ink/10 bg-surface-1 shadow-[0_16px_48px_rgb(var(--shadow-ambient)/calc(0.5*var(--shadow-scale)))] animate-fade-up">
          {/* Infos utilisateur */}
          ...
          {/* Statut sync */}
          <div className="border-b border-ink/5 px-4 py-3 flex items-center gap-2.5">
            {syncStatus === 'syncing' ? (
            ...
            <span className="text-xs text-ink/70">{SYNC_LABEL[syncStatus]}</span>
          </div>
          ...
          <div className="p-2">
            {!confirmingSignOut ? (
              ...
              <div className="flex flex-col gap-2 px-1 py-1">
                <p className="text-xs text-ink/60 px-2">Tu seras redirigé vers la page de connexion.</p>
